...
# D:\download\ICAFusion-main\ICAFusion-main\utils\datasets.py (大约 390 行)

def img2label_paths(img_paths):
    t = []
    # 强制定义替换变量：将 'images' 文件夹替换为 'labels' 文件夹
    # 这样可以匹配大多数 YOLO 风格的路径列表
    sa = 'images'
    sb = 'labels'

    for x in img_paths:
        # 1. 替换文件夹名：.../images/... -> .../labels/...
        # 注意：这里使用 x.replace(sa, sb, 1) 来替换第一个匹配项
        label_path = x.replace(sa, sb, 1)

        # 2. 替换文件扩展名：.../xxx.jpg -> .../xxx.txt
        # 'txt'.join(y.rsplit(y.split('.')[-1], 1)) 是 YOLOv5 替换扩展名的方法
        t.append('txt'.join(label_path.rsplit(label_path.split('.')[-1], 1)))
