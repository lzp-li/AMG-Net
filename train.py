...
# === [修改 1] 导入配置注入函数 ===
from models.common import set_fusion_config
...
def train_rgb_ir(hyp, opt, device, tb_writer=None):
    ...
    if 'model_flags' in data_dict:
        set_fusion_config(data_dict['model_flags'])  # 关键：这一步把开关传给了 common.py
        deterministic_mode = data_dict['model_flags'].get('deterministic', True)

    # 初始化种子 (动态决定是否开启确定性)
    init_seeds(seed=1 + rank, deterministic=deterministic_mode)

    if not deterministic_mode:
        logger.info("⚡ [Speed] Deterministic mode DISABLED. Enabling CUDNN Benchmark for speed.")
        torch.backends.cudnn.benchmark = True  # 开启加速
    else:
        logger.info("🔒 [Reproduce] Deterministic mode ENABLED.")
    ################################
    ...
    labels_path = data_dict['path'] + '/labels/test_rgb'
    ...
    # Resume
    start_epoch, best_fitness = 0, 0.0 #map越大越好
    #start_epoch, best_fitness = 0, 1.0 # MR 越小越好，初始值设为 1.0
    if pretrained:
        # Optimizer
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if rank in [-1, 0]:
            ...
            if fi > best_fitness:
                best_fitness = fi
            # Update best MR (Miss Rate)
            # fi = MRresult[0]
            # if best_fitness == 0.0 and fi > 0:
            #     best_fitness = 1.0
            # if fi < best_fitness:
            #     best_fitness = fi
...
if __name__ == '__main__':
    ...
    parser.add_argument('--name', default='000（去除TGD和DPCF）', help='save to project/name')
